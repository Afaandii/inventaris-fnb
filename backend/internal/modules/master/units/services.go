...
type ServiceUnit interface {
	...
	Create(unit_code, unit_name, types, short_name string, is_active bool) (*model.Units, error)
	Update(id_unit uint, unit_code, unit_name, types, short_name string, is_active bool) (*model.Units, error)
...
func (us *serviceUnit) Create(unit_code, unit_name, types, short_name string, is_active bool) (*model.Units, error) {
	unt := &model.Units{
		UnitCode:  unit_code,
		UnitName:  unit_name,
		Type:      types,
...
func (us *serviceUnit) Update(id_unit uint, unit_code, unit_name, types, short_name string, is_active bool) (*model.Units, error) {
	unt, err := us.repo.GetById(uint(id_unit))
	if err != nil {
		return nil, err
	}

	unt.UnitCode = unit_code
